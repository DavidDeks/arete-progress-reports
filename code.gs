    ...
    if (scoreSheetsFiles.length != level.length) {
      ...
      if (sendQuota < gymnastCount) {
        ...
        if (response == ui.Button.NO) {
          ...
          // gymnast = {fullName:'', parentName:'', email:'', level: '', scores:[], tableContent:''};
          levelLoop:
          for (var i = 0; i < scoreSheets.length; i++) {
            scoresLoop:
            for (var j = 1; j < scoreSheets[i].length; j++) { // Start at 1 to skip header row
              var currentGymnast = gymnastEmails.filter( function(gymnast){return (gymnast.fullName.toUpperCase()==scoreSheets[i][j][2].toUpperCase().trim());} ); // Returns an array with just the one gymnast object element.
          ...
          if (!currentGymnast[0]) { // Gymnast name not found in email list
...
function generateTableContent(sheet, row) { // Takes the 2D sheet array as a parameter
  ...
  for (var i = 3; i < nbrOfCols; i++) {
    ...
    if (currentCell.trim().toUpperCase().includes('COMMENT')) {
      ...
      var td = currentCell.slice(currentCell.indexOf('[') + 1, -1);
